...
/**
 * add the lilyaudio shortcode to nunjucks inside markdown
 * @param {Object} nunjucksEngine the context of the extension
 * @return {String} html chunk to render in markdown
 */
function lilyaudioExtension(nunjucksEngine) {
  return new function() {
    ...
    this.parse = function(parser, nodes, lexer) {
      ...
      const body = parser.parseUntilBlocks('endlilyaudio');
    ...
    this.run = function(context, param, template, height, audio, body, callback) {
      ...
      const tempString = `
          ...
          ${body()}`;
      const hash = md5(tempString); // need a better caching strategy
      const lilypondWrapper = `<div class="lilypond-container"><div class="lilypond" onclick="togglePlayAudio('${hash}')">`;
      const audioMarkup = audio === 'none'
      ...
      if (isCached) {
        ...
        if (param === 'inline') {
          fs.readFile(`${directory}/${hash}.min.preview.svg`, function(err, data) {
            ...
            const ret = new nunjucksEngine.runtime.SafeString(minifyHtml(lilypondWrapper + String(data) + audioMarkup));
            callback(null, ret);
          });
        } else {
        ...
        fs.writeFile(`${directory}/${hash}.ly`, tempString, function(err) {
          ...
          exec(`lilypond -dpreview -dbackend=svg --output=${directory} ${directory}/${hash}.ly`, function(err, stdout, stderr) {
            ...
            if (audio !== 'none') {
              ...
              for (const i in audioFileExtensions) {
                const ext = audioFileExtensions[i];
